...
def save_ekf_data_to_csv(csv_writer, iteration, u, y, xEst_before, PEst_before, 
                         xEst_after, PEst_after, execution_time):
    """
    Save EKF SLAM iteration data to CSV
    Format: iteration, execution_time, u[0], u[1], num_obs, [obs_data], 
            xEst_before_size, [xEst_before], PEst_before_size, [PEst_before_flatten],
            xEst_after_size, [xEst_after], PEst_after_size, [PEst_after_flatten]
    """
    row = [iteration, execution_time]
    
    # Control input u
    row.extend([u[0, 0], u[1, 0]])
    
    # Observations
    num_obs = len(y)
    row.append(num_obs)
    for i in range(num_obs):
        row.extend([y[i, 0], y[i, 1], int(y[i, 2])])
    
    # State before
    row.append(len(xEst_before))
    row.extend(xEst_before.flatten().tolist())
    
    # Covariance before (flattened)
    row.append(PEst_before.shape[0])
    row.extend(PEst_before.flatten().tolist())
    
    # State after
    row.append(len(xEst_after))
    row.extend(xEst_after.flatten().tolist())
    
    # Covariance after (flattened)
    row.append(PEst_after.shape[0])
    row.extend(PEst_after.flatten().tolist())
...
def main():
    ...
    # Open CSV file for recording EKF data
    csv_file = open('ekf_slam_data.csv', 'w', newline='')
    csv_writer = csv.writer(csv_file)
    
    # Write header
    csv_writer.writerow(['# EKF SLAM Data Export'])
    csv_writer.writerow(['# Format: iteration, execution_time(s), u[0], u[1], num_observations, [y_data...], '
                        'xEst_before_size, [xEst_before...], PEst_before_size, [PEst_before_diag...], '
                        'xEst_after_size, [xEst_after...], PEst_after_size, [PEst_after_diag...]'])
    
    # Save initial conditions
    with open('ekf_slam_initial_conditions.txt', 'w') as f:
        f.write("# Initial Conditions\n")
        f.write(f"STATE_SIZE={STATE_SIZE}\n")
        f.write(f"LM_SIZE={LM_SIZE}\n")
        f.write(f"DT={DT}\n")
        f.write(f"MAX_RANGE={MAX_RANGE}\n")
        f.write(f"M_DIST_TH={M_DIST_TH}\n")
        f.write("\n# Landmarks [x, y]\n")
        np.savetxt(f, Landmarks, fmt='%.6f')
        f.write("\n# Initial xEst\n")
        np.savetxt(f, xEst, fmt='%.6f')
        f.write("\n# Initial PEst\n")
        np.savetxt(f, PEst, fmt='%.6e')
        f.write("\n# Q matrix\n")
        np.savetxt(f, Q, fmt='%.6e')
        f.write("\n# Py matrix\n")
        np.savetxt(f, Py, fmt='%.6e')
    ...
    while time_sim <= SIM_TIME:
        ...
        # Store state before EKF update
        xEst_before = xEst.copy()
        PEst_before = PEst.copy()

        # Time the EKF execution
        start_time = time.perf_counter()
        xEst, PEst = ekf_slam(xEst, PEst, u, y)
        execution_time = time.perf_counter() - start_time

        # Save data to CSV
        save_ekf_data_to_csv(csv_writer, iteration, u, y, xEst_before, PEst_before, 
                            xEst, PEst, execution_time)
        
        # Optionally save full covariance matrices (can be large!)
        # Uncomment if you need full covariance matrices
        # if iteration % 10 == 0:  # Save every 10th iteration
        #     save_full_covariance_to_file('ekf_slam_full_covariance.txt', iteration, PEst)
    ...
    # Close CSV file
    csv_file.close()
    print(f"\nEKF SLAM data saved to: ekf_slam_data.csv")
    print(f"Initial conditions saved to: ekf_slam_initial_conditions.txt")
