...
int main() {
    std::cout << "\n========================================" << std::endl;
    std::cout << "EKF SLAM C++ Embedded Data Testbench" << std::endl;
    std::cout << "========================================\n" << std::endl;

    // Load data using embedded header
    EKFSLAMDataLoaderEmbedded loader;

    std::cout << "Loading initial conditions from embedded header..." << std::endl;
    if (!loader.loadInitialConditionsFromHeader()) {
        std::cerr << "Failed to load initial conditions from header" << std::endl;
        return 1;
    }

    std::cout << "Loading iteration data from embedded header..." << std::endl;
    if (!loader.loadIterationDataFromHeader()) {
        std::cerr << "Failed to load iteration data from header" << std::endl;
        return 1;
    }
